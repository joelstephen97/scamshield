...
**Feed rules** — the blocklist is built in [parry-feed](https://github.com/joelstephen97/scamshield-feed); open issues/PRs there for sources or FP-guard changes. `rules/blocklist.json` here is a snapshot produced by `node build.js --snapshot` in that repo.
