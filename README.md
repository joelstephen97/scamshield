...
Every push and pull request also runs the [CI workflow](.github/workflows/ci.yml): unit tests on Linux (Node 22 with coverage floors, Node 24) and Windows, both builds with a package-content and permissions guard, `web-ext lint` on the Firefox package, the Playwright suite sharded three ways against the unpacked Chrome zip, the relay tests, `npm audit`, a gitleaks history scan, a dependency review on pull requests, and actionlint on the workflows themselves; [CodeQL](.github/workflows/codeql.yml) runs alongside it and [Dependabot](.github/dependabot.yml) sends grouped weekly updates. The single status to require on `main` is `CI / ci-ok`. Every release zip carries a build-provenance attestation: `gh attestation verify scamshield-chrome.zip --repo joelstephen97/scamshield`.
