...
function validatePayload(p) {
  ...
  if (typeof p.level !== 'string' || p.level.length > 16 || typeof p.score !== 'number') return { ok: false, error: 'level/score' };
  ...
  if (p.pageFeatures !== null) {
    ...
    const ks = Object.keys(f.tokens); if (ks.length > 2000 || !ks.every((k) => /^\d+$/.test(k) && Number(k) < 32768 && typeof f.tokens[k] === 'number')) return { ok: false, error: 'tokens' };
  }
  if (!Array.isArray(p.iconMatches) || p.iconMatches.length > 6 || !p.iconMatches.every((m) => m && typeof m.brand === 'string' && m.brand.length <= 32 && typeof m.distance === 'number')) return { ok: false, error: 'iconMatches' };
  if (typeof p.extVersion !== 'string' || p.extVersion.length > 16 || typeof p.ts !== 'number') return { ok: false, error: 'meta' };
  return { ok: true };
}
