...
_Last updated: 2026-09-07 · applies to ScamShield 0.13.0 and later (Chrome Web Store listing `fojjjofjimbfoddafoampojopijnlihl`, Firefox build, and source builds from [github.com/joelstephen97/scamshield](https://github.com/joelstephen97/scamshield))._

_What changed in 0.13.0: ScamShield now downloads one additional file — a small "hot list" of domains reported for phishing in the last 48 hours — once an hour, alongside the 12-hourly threat-list download it has always done (see section 3). **This does not change what data ScamShield collects, stores or sends**: it is the same static, public file for every user, the request says nothing about you or your browsing, and nothing is uploaded. The release also adds a "Trust this site" button to every warning, which writes only to the allowlist already stored in your own browser. Community reporting is still off by default, and the payload it sends when you opt in is unchanged._
...
| Threat-list download | On | Downloads the same public block list as every other user — the main file every 12 hours, a small 48-hour "hot list" every hour. Nothing about you is sent. Can be disabled. |
