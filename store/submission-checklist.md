...
- [ ] **Store listing (English):** REPLACE the whole Title / Summary / Description with the `## Name` / `## Short description` / `## Full description` + `## What's new (0.11.0)` sections of `store/listings/en.md` — the description currently live on the item is the 0.5-era text that Google rejected; do not merge, overwrite. Category *Privacy & Security* (or *Productivity → Tools*); language English; homepage `https://joelstephen97.github.io/scamshield/`; support URL `https://github.com/joelstephen97/scamshield/issues`.
